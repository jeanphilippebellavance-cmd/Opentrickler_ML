...
const eeprom_motor_data_t default_motor_data = {
    ...
    .motor_data[0] = {
        ...
        .current_ma = 800,                  // mA RMS for performance; 17HS2408 nominal is 600 mA/phase
        .microsteps = 256,                  // Maximum microstep that TMC2209 supports
        .max_speed_rps = 8,                 // Hardware exploration limit for the Memphis V2 coarse tube
    ...
    .motor_data[1] = {
        ...
        .current_ma = 800,                  // mA RMS for performance; 17HS2408 nominal is 600 mA/phase
        .microsteps = 256,                  // Maximum microstep that TMC2209 supports
        .max_speed_rps = 6,                 // Lets AI characterize faster fine-flow without profile caps
        .r_sense = 110,                     // Default sensing resistor (110ohm is used for stanard TMC2209 modules)
...
static bool motor_config_apply_memphis_v2_nema17_envelope(eeprom_motor_data_t *motor_data) {
    if (motor_data == NULL) {
        return false;
    }
    ...
    // Older EEPROM configs capped both motors at 5 rps. For the 17HS2408 Memphis V2
    // setup, allow AI characterization to explore the useful hardware envelope.
    if (motor_data->motor_data[0].max_speed_rps < default_motor_data.motor_data[0].max_speed_rps) {
        motor_data->motor_data[0].max_speed_rps = default_motor_data.motor_data[0].max_speed_rps;
        changed = true;
    }
    if (motor_data->motor_data[1].max_speed_rps < default_motor_data.motor_data[1].max_speed_rps) {
        motor_data->motor_data[1].max_speed_rps = default_motor_data.motor_data[1].max_speed_rps;
        changed = true;
    }

    if (motor_data->motor_data[0].full_steps_per_rotation == 0) {
        motor_data->motor_data[0].full_steps_per_rotation = 200;
        changed = true;
    }
    if (motor_data->motor_data[1].full_steps_per_rotation == 0) {
        motor_data->motor_data[1].full_steps_per_rotation = 200;
        changed = true;
    }
...
bool motor_config_init(void) {
    ...
    bool motor_config_upgraded = motor_config_apply_memphis_v2_nema17_envelope(&eeprom_motor_data);
    ...
    if (motor_config_upgraded) {
        (void)motor_config_save();
    }
...
void apply_rest_motor_config(motor_config_t * motor_config, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "m0") == 0) {
            ...
            direction_config_changed = motor_config->persistent_config.inverted_direction != inverted_direction;
    ...
    /*
     * Let direction changes take effect immediately when the motor is idle.
     * Without this, changing m9 from the web UI can look like it failed until
     * a reboot because the live DIR pin still holds the old idle direction.
     */
    if (direction_config_changed && fabsf(motor_config->prev_velocity) < 0.001f) {
        motor_config->step_direction = motor_config->persistent_config.inverted_direction ? true : false;
        gpio_put(motor_config->dir_pin, motor_config->step_direction);
    }
}
